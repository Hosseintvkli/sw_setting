# sw_modbus_ext_code_generator

## Software Version

- **Generator Version:** 3.0.11  


## Outputs

The application generates the following files:

### TXT Files
| File Name | Description |
|-----------|-------------|
| parameter_list_atlas_main - Internal.txt | - |
| parameter_list_atlas_main - Info.txt | - |
| parameter_list_atlas_main - DefaultUnit.txt | - |
| parameter_list_atlas_main - ParameterList.txt | - |
| parameter_list_atlas_main - sDownStreamStatistics.txt | - |
| parameter_list_atlas_main - sDownStreamSetting.txt | - |
| parameter_list_atlas_main - eTag1_SettingGroup.txt | - |
| parameter_list_atlas_main - sProfilerData.txt | - |
| parameter_list_atlas_main - sProfilerSetting.txt | - |
| parameter_list_atlas_main - sBoardStartupReport.txt | - |
| parameter_list_atlas_main - sBoardStartupReportItem.txt | - |
| parameter_list_atlas_main - sSensorAccXrza3071Data.txt | - |
| parameter_list_atlas_main - sSensorAccXrza3071Setting.txt | - |
| parameter_list_atlas_main - sSensorAccSnjData.txt | - |
| parameter_list_atlas_main - sSensorAccSnjSetting.txt | - |
| parameter_list_atlas_main - sSensorAd7177Data.txt | - |
| parameter_list_atlas_main - sSensorAd7177Setting.txt | - |
| parameter_list_atlas_main - sSensorAd7177ChannelSetting.txt | - |
| parameter_list_atlas_main - sSensorFogData.txt | - |
| parameter_list_atlas_main - sSensorFogSetting.txt | - |
| parameter_list_atlas_main - s3d.txt | - |
| parameter_list_atlas_main - s3dI32.txt | - |
| parameter_list_atlas_main - eFogType.txt | - |
| parameter_list_atlas_main - eAccType.txt | - |
| parameter_list_atlas_main - s3dI16.txt | - |
| parameter_list_atlas_main - eSnjBoardVersion.txt | - |
| parameter_list_atlas_main - eSnjThermometerType.txt | - |
| parameter_list_atlas_main - eConnectedDeviceHostMode.txt | - |
| parameter_list_atlas_main - sFilterSetting.txt | - |
| parameter_list_atlas_main - sInternalImuData.txt | - |
| parameter_list_atlas_main - sInternalImuSetting.txt | - |
| parameter_list_atlas_main - eInternalImuSerialPort.txt | - |
## Notes

- The outputs are generated based on the version **3.0.11** of this software.  

---

*Generated on: 2026-08-29 - 16:10:54*  
