 ...
 */
module.exports = function generateModbusDatabaseVirtualParametersIdJsonFile(model) {
  let output = ``;
  let fileName = ``;
  let outputObject;

  outputObject = codeGenCommon.unwrapCellValues(model.eVirtualParameterIdSheet);
  ...
  outputObject = outputObject.map((row) => {
    ...
    Object.keys(row).forEach((field) => {
      switch(field)                 {
        // case `isVirtualParameterId` :
        // case `NameUpperSnakeCase`   :

        case `Value`                :
        case `Size`                 : { newRow[field] = codeGenCommon.convNumStringToNum(row[field], {expectedType: `number`}); } break;
  ...
  fileName += `cg_database_virtual_parameters_id.json`;
