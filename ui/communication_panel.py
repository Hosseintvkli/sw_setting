...
class CommunicationSettingsPanel(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        communication_type_group = QGroupBox("Communication Type")
        communication_type_group.setLayout(kind_row)

        self.serial_settings_group_box = QGroupBox("Modbus Serial Connection")
        ...
        self._remove_spin_buttons(
            self.spin_box_custom_baud_rate_bits_per_second
        )
        ...
        self.combo_box_local_network_interface.setMaxVisibleItems(12)
        self.combo_box_local_network_interface.view().setTextElideMode(
        ...
        ethernet_local_form = QFormLayout(self.ethernet_settings_group_box)
        ethernet_local_form.addRow("Interface name:", network_controls)
        ethernet_local_form.addRow("", self.label_local_network_validation)
        ethernet_local_form.addRow(
            "", self.push_button_open_windows_network_connections
        )

        self.modbus_tcp_settings_group_box = QGroupBox("Modbus TCP Connection")
        ...
        layout.setSpacing(4)
        layout.addWidget(communication_type_group)
        ...
        self.push_button_refresh_network_interface_list.setObjectName(
            "secondaryActionButton"
        )
    ...
    def _remove_spin_buttons(spin: QSpinBox) -> None:
        """Numeric communication fields are typed; arrow controls add clutter."""
    ...
    def _update_link_kind_visibility(self, *_args) -> None:
        ...
        self.modbus_tcp_settings_group_box.setVisible(not serial)
    ...
    def set_available_network_interfaces(
        ...
        for interface in selected_interfaces:
            ...
            adapter_name = description or name
            label = f"{adapter_name}  —  {ipv4}" if ipv4 else adapter_name
            self.combo_box_local_network_interface.addItem(
                label, (name, ipv4)
            )
            item_index = self.combo_box_local_network_interface.count() - 1
            details = [f"Windows interface: {name}"]
            if description:
                details.append(f"Adapter: {description}")
            if ipv4:
                details.append(f"IPv4: {ipv4}")
            self.combo_box_local_network_interface.setItemData(
                item_index,
                "\n".join(details),
    ...
    def _update_network_combo_tooltip(self, *_args) -> None:
        index = self.combo_box_local_network_interface.currentIndex()
        tooltip = self.combo_box_local_network_interface.itemData(
            index, Qt.ItemDataRole.ToolTipRole
        )
        self.combo_box_local_network_interface.setToolTip(str(tooltip or ""))
