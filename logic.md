...
تب Monitoring: درختی مشابه Setting با سطح اول Tag2 و ادامهٔ مسیر از Name؛ Checkbox فقط روی برگ پارامتر است. Select All در حالت بدون انتخاب همه را فعال می‌کند و در حالت انتخاب جزئی/کامل، Clear All همهٔ برگ‌های فعلی را غیرفعال می‌کند. دکمهٔ صریح Start/Stop وجود دارد و ورود به تب به‌تنهایی هیچ readای ایجاد نمی‌کند. خواندن فقط پس از Start، برای برگ‌های تیک‌خورده و تا وقتی تب فعال است انجام می‌شود؛ خروج از تب چرخه را متوقف می‌کند. برای جلوگیری از فرمان بسیار بلند و نمایش تدریجی نتیجه، انتخاب‌های بزرگ در فرمان‌های حداکثر ۲۵۰ پارامتری می‌چرخند. انتخاب‌ها با ParameterId بین Deviceها حفظ می‌شوند؛ تغییر مقدار ۳ ثانیه سبز و قرائت قدیمی پس از ۵ ثانیه dim می‌شود و تایمر بصری فقط ردیف‌های داخل viewport را بازآرایی می‌کند.
